...
class ModelDownloadService {
  ...
  static const String _vocabUrl =
      'https://github.com/SayanMz/notepad-app/releases/download/2.5.0/vocab.txt';
  ...
  static Future<bool> startBackgroundDownload() async {
    ...
    try {
      ...
      // Download vocab atomically to prevent partial/corrupted files on network interruption
      final vocabFile = File(join(baseDir.path, 'models', 'vocab.txt'));
      if (!vocabFile.existsSync()) {
        await vocabFile.parent.create(recursive: true);

        final vocabTmp = File('${vocabFile.path}.tmp');
        if (vocabTmp.existsSync()) await vocabTmp.delete();

        final client = HttpClient()
          ..connectionTimeout = const Duration(seconds: 15);
        try {
          final req = await client.getUrl(Uri.parse(_vocabUrl));
          final res = await req.close();
          if (res.statusCode == 200) {
            await res.pipe(vocabTmp.openWrite());
            await vocabTmp.rename(vocabFile.path);
          } else {
            throw HttpException(
              'Failed to download vocab: status ${res.statusCode}',
            );
          }
        } finally {
