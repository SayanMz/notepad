...
class NoteDocumentService {
    ...
    pdf.addPage(
      pw.MultiPage(
        build: (context) => [
          ...
          ..._buildPdfBlocks(lines, fontReg, fontBold, fontItalic, fontBoldItalic, emojiFont),
        ],
      ),
    );
    stopwatch.stop();
  ...
  static List<pw.Widget> _buildPdfBlocks(
    ...
    for (final line in lines) {
      ...
      widgets.add(_buildPdfLine(line, orderedListIndex, fontReg, fontBold, fontItalic, fontBoldItalic, emojiFont));
  ...
  static pw.Widget _buildPdfLine(
    ...
    final textBlock = pw.Container(
      ...
      padding: pw.EdgeInsets.only(
        left: (isBullet || isOrdered || isBlockquote) ? 12 : (isCodeBlock ? 8 : 0),
        top: isCodeBlock ? 6 : 0,
  ...
  static pw.TextSpan _pdfTextSpan(
    ...
    return pw.TextSpan(
      ...
      style: pw.TextStyle(
        ...
        color: isLink ? PdfColors.blue700 : _pdfColor(attributes['color'] as String?),
        decoration: isLink ? pw.TextDecoration.underline : _pdfDecoration(attributes),
      ),
  ...
  static String _htmlSpan(_InlineRun run) {
    ...
    if (attributes['underline'] == true) styles.add('text-decoration: underline;');
    if (attributes['strike'] == true) styles.add('text-decoration: line-through;');
    if (attributes['size'] != null) styles.add('font-size: ${attributes['size']}px;');
    if (attributes['background'] != null) styles.add('background-color: ${attributes['background']};');
