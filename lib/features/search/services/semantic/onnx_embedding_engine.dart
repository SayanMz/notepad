...
class OnnxEmbeddingEngine {
  ...
  /// Locates local model and vocabulary files on disk across external storage, documents directory, or system temp.
  static Future<Map<String, File>> getLocalModelFiles() async {
    // Check Android external storage directory for downloaded ONNX binary and vocab file.
    if (!kIsWeb && Platform.isAndroid) {
      try {
        final extDir = await getExternalStorageDirectory();
        if (extDir != null) {
          final modelFile = File(join(extDir.path, 'models', _modelFileName));
          final vocabFile = File(join(extDir.path, 'models', _vocabFileName));
          if (modelFile.existsSync() && vocabFile.existsSync()) {
            return {'model': modelFile, 'vocab': vocabFile};
          }
        }
      } catch (e) {
    ...
    try {
      ...
      return {
        'model': File(join(appDocsDir.path, 'models', _modelFileName)),
        'vocab': File(join(appDocsDir.path, 'models', _vocabFileName)),
      };
    } catch (_) {
      final tempDir = Directory.systemTemp;
      return {
        'model': File(join(tempDir.path, 'models', _modelFileName)),
        'vocab': File(join(tempDir.path, 'models', _vocabFileName)),
      };
    }
  }

  /// Returns whether model files exist on disk.
  static Future<bool> isModelAvailable() async {
    try {
      final files = await getLocalModelFiles();
      return files['model']!.existsSync() && files['vocab']!.existsSync();
    } catch (_) {
      return false;
    }
  }
  ...
  static Future<void> init() async {
    ...
    try {
      ...
      final files = await getLocalModelFiles();
      _session = OrtSession.fromFile(files['model']!, OrtSessionOptions());
      final vocabData = await files['vocab']!.readAsString();
